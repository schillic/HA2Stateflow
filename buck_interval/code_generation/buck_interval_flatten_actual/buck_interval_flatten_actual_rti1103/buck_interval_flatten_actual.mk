# =============================================================================
#  FILE:
#    buck_interval_flatten_actual.mk
#
#  DESCRIPTION:
...
#    J. Hagendorf
#
#  (c) Copyright 2011, dSPACE GmbH. All rights reserved.
#
#  $RCSfile: buck_interval_flatten_actual.mk $ $Revision: 2.74 $ $Date: 2014/02/25 16:00:13MEZ $
#  $ProjectName: e:/ARC/Products/ImplSW/RTISW/RTI/Components/Build/Makefile/project.pj $
# =============================================================================
# NEVER CHANGE THIS LINE:
# RTI_TEMPLATE_MAKEFILE_VERSION = 220
# =============================================================================
...
MAKEFILE                       = buck_interval_flatten_actual.mk
BUILDARGS                      =  EXTMODE_STATIC_ALLOC=0 TMW_EXTMODE_TESTING=0 EXTMODE_STATIC_ALLOC_SIZE=1000000 EXTMODE_TRANSPORT=0 MAT_FILE=0 NCLIENT="OPTION_DISABLED"
...
COMPUTER                       = PCWIN64
MODEL                          = buck_interval_flatten_actual
MODULES                        = buck_interval_flatten_actual_data.c buck_interval_flatten_actual_trc_ptr.c rtGetInf.c rtGetNaN.c rt_nonfinite.c 
S_FUNCTIONS                    = 
S_FUNCTIONS_LIB                = 
SOLVER                         = 
NUMST                          = 2
...
_TMP_OPTION_NAME                 =   PLATFORM
$(_TMP_OPTION_NAME)              = DS1103
...
CMDLOADER   = "$(DSPACE_ROOT)\Exe\CmdLoader\bin\CmdLoader.exe"
COFFCONV    = "$(DSPACE_ROOT)\Exe\coffconv.exe"
...
SG  = "$(DSPACE_ROOT)\exe\DsSetUuid.exe"
...
%  set DSPACE_RTLIB_ADDLIBS := $(DSPACE_RTLIB_ADDLIBS_SPEC,*F,M\.lib$$,NDs1103SlvdspFwPrg.lib$$,Nds$(BOARD_TYPE)dbg.lib$$,Nds$(BOARD_TYPE).lib$$)
...
#
MSG_OPTS = -nQi -QmsC0001 -Qms0111 -QmsC0177 -QmsC0186 -QmsC0188 -QmsC0191 -QmsC0193 -QmeC0223 -QmeC0233 -QmsC0236 -QmsC0237 -QmwC0260 -QmsC0340 -QmsC0381 -QmsC0514 -Qms0550 -QmsC0826 -Qms1430 -Qms1457
...
RTWCUST_INCLUDES = \
	$(START_DIR)\buck_interval_flatten_actual_rti1103 \
	$(START_DIR)
...
LNK_OPTS = -K0 -QmsA0407 -QmsA0314 -QmeA0307 -QmeA0329
...
%  if defined(_COMMAND_LOAD)
make_main : start_check                                            download make_success_quiet
%  else     # _COMMAND_LOAD
make_main : start_check init slave_opt compile_deps link             cleanup download make_success
%  endif    # _COMMAND_LOAD
%else     # _IS_ROOT_MODEL
...
start_check :
	...
	%endif
	%if !(exists("$(SG,"=)"))
	  %echo ERROR: Cannot find $(SG,F) tool. Possible reasons are:
	...
	%ifdef PLATFORM
	  %if "$(PLATFORM)" == ""
	    %echo ERROR: Make option PLATFORM is not set or its value is empty.
	    %do make_abort
	  %endif
	%endif
	%ifdef LOAD
	  %if !member($(LOAD), ON OFF)
	    %echo ERROR: Invalid LOAD option '$(LOAD)' specified.
	    %echo        Select ON or OFF.
	    %do make_abort
	  %endif
	%endif
	%ifdef SSTATE
...
make_init :
	%ifdef _IS_ROOT_MODEL
	  %do delete_file FILE_LIST="$(PROGRAM,$(WS_PACK_TARGET_ARGS)) $(MAPFILE,$(WS_PACK_TARGET_ARGS)) $(MODEL).rtimakefinish"
	%endif
	%ifdef COMPALL
	  %if '$(COMPALL, UC)' == 'ON'
...
# -----------------------------------------------------------------------------
# Treatment of slave option (RTI1103 only)
# -----------------------------------------------------------------------------

slave_opt :
	# SLAVE
	%ifdef SLAVE
	  # Usage:     coffconv obj_file [options] [/?] 
	  #   /c              generate *.slc c-module with loader function 
	  #   /n              disable beep on error 
	  #   /o output_file  ouput file name 
	  #   /q              quiet mode, only error messages will be displayed 
	  #   /t board_type   taget board type, DS2210 (default), DS2211, DS2302, DS4302, DS1103, DS1104 
	  %set SLAVE_NO_SPACE = $(SLAVE,W)      
	  %set SLAVE_OBJECT = $(SLAVE_NO_SPACE,B)                         
	  $(EXEC_SHELL) $(COFFCONV) "$(SLAVE)" /t DS$(BOARD_TYPE) /c /n /q -o Slv1103_$(SLAVE_OBJECT).slc
	  %if $(status)
	    %set _STATUS = $(status)
	    %echo CREATING C-MODULE FROM SLAVE OBJECT $(SLAVE) FAILED ($(_STATUS))
	    %do make_abort
	  %endif
	%endif
...
link : link_main blockset_after_linkage link_finish
...
link_main :
	...
	%endif
	%do link_program
	%do postlink_update_uuid
	%echo LINKING FINISHED

postlink_update_uuid:
    #
    # Update uuid in sdf and application image file
    #
    %if exists($(SDFFILE)) && exists($(PROGRAM))
        # /rtp rtp-name   specifies rtp-name for sdf file
        # /sdf sdf-file   specifies sdf-file to be updated
        # /n              no beep on error
        # /q 1            quiet mode 1 :  no info messages
        # /upo            update UUID in object file
        $(EXEC_SHELL) $(SG) $(PROGRAM) /n /q 1 /upo /sdf $(SDFFILE) /rtp Master
        %if $(status)
          %set _STATUS = $(status)
          %echo UPDATING UUID IN $(PROGRAM,F,UC) AND $(SDFFILE,F,UC) FAILED ($(_STATUS))
          %do make_abort
        %endif
    %endif
...
link_finish :
	# empty file marking the end of the RTI make process
	%echo > $(MODEL).rtimakefinish
...
download :
	...
	%if ('$(LOAD_OPT)' == 'ON') || ('$(LOAD_OPT)' == 'FLASH')
	  ...
	  # PLATFORM
      %set LOAD_MSG := $(LOAD_MSG)$(NEWLINE)  PLATFORM:      $(PLATFORM)
	  %echo
	  %echo LOADING APPLICATION "$(SDFFILE,F)" $(LOAD_MSG)                
	  --$(CMDLOADER) -ol 2 $(FLASH_OPT) $(PLATFORM_OPT) $(SDFFILE)
	  %if $(status)
	...
	%ifndef _IS_MP_BUILD
	  ...
	  %if exists($(SIMENGINE_OPTS_MARKER_FILE))
	    ...
	    %if "$(SIMENGINE_OPTS_MARKER_FILE_CONTENS)" != "$(SIMENGINE_OPTS_MARKER_CURRENT)"
	      # Options changed compared to prevoius build.
