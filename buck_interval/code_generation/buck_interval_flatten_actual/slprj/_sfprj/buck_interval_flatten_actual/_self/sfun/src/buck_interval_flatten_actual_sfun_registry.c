...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_buck_interval_flatten_actual_process_check_sum_call
    ( int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_buck_interval_flatten_actual_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_buck_interval_flatten_actual_autoinheritance_info( int
  ...
  if (strcmp(machineName, "buck_interval_flatten_actual") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_buck_interval_flatten_actual_autoinheritance_info(nlhs,plhs,3,
      newRhs);
  }
...
{
  extern unsigned int sf_buck_interval_flatten_actual_third_party_uses_info( int
    nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "buck_interval_flatten_actual") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_buck_interval_flatten_actual_third_party_uses_info(nlhs,plhs,3,
      newRhs);
  }
...
unsigned int sf_process_get_updateBuildInfo_args_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_buck_interval_flatten_actual_updateBuildInfo_args_info(
    int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "buck_interval_flatten_actual") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_buck_interval_flatten_actual_updateBuildInfo_args_info(nlhs,plhs,3,
      newRhs);
  }
...
{
  extern unsigned int
    sf_buck_interval_flatten_actual_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "buck_interval_flatten_actual") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_buck_interval_flatten_actual_get_eml_resolved_functions_info(nlhs,
      plhs,2,newRhs);
  }
...
extern unsigned int sf_buck_interval_flatten_actual_method_dispatcher(SimStruct *
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"buck_interval_flatten_actual")) {
    return(sf_buck_interval_flatten_actual_method_dispatcher(simstructPtr,
...
extern void buck_interval_flatten_actual_terminator(void);
void sf_machine_global_terminator(void)
{
  sfGlobalMdlStartCallCounts--;
  if (sfGlobalMdlStartCallCounts == 0) {
    buck_interval_flatten_actual_terminator();
...
extern void buck_interval_flatten_actual_initializer(void);
extern void buck_interval_flatten_actual_register_exported_symbols(SimStruct* S);
extern void buck_interval_flatten_actual_debug_initialize(struct
  SfDebugInstanceStruct*);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  buck_interval_flatten_actual_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfGlobalMdlStartCallCounts == 1) {
    ...
    if (!simModeIsRTWGen) {
      buck_interval_flatten_actual_debug_initialize(sfGlobalDebugInstanceStruct);
    }

    buck_interval_flatten_actual_initializer();
  }
